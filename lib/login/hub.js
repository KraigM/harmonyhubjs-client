...
var injectSaslAuth = require('./linksaslauth')
...
function getIdentity (hubhost, hubport, authToken) {
  ...
  var isHarmonyLink = authToken ? true : false;
  ...
  if (isHarmonyLink) {
    injectSaslAuth(xmppClient.availableSaslMechanisms);
  }
 ...
 *     (int) hubport - Optional. Port of the Harmony hub to connect. By default,
 *                     this is set to 5222.
 *     (String) userAuthToken - Optional. A authentication token, retrieved from logitechs
 *                              web service.
 *
 * Returns:
